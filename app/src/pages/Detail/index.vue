...
export default {
  ...
  methods: {
    ...
    async addShopCar() {
      //这行代码其实就是在  调用仓库中的addOrUpdateShopCart
      //因为仓库中的addOrUpdateShopCart，加入了async await，而这里只需要只要成功与失败
      //所以也要加上async await
      //try catch  成功了干嘛，失败了干嘛
      try {
        ...
        //路由跳转
        //像这个简单的数据skuNum可以通过query参数传递
        //复杂的数据 skuInfo通过会话储存 （关闭窗口就删除储存）
        //第一个是存储的名字，第二个是存储的值，JSON.stringify把值转为字符串
